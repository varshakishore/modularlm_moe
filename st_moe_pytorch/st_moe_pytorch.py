...
class Expert(nn.Module):
    def __init__(self, config, expert_init_weights=None):
        super().__init__()
        ...
        if expert_init_weights is not None:
            self.c_fc.weight.data = expert_init_weights[0]
            self.c_fc.bias.data = expert_init_weights[1]
            self.c_proj.weight.data = expert_init_weights[2]
            self.c_proj.bias.data = expert_init_weights[3]
...
class MoE(Module):
    ...
    def __init__(self,
        ...
        experts = default(experts, lambda: [Expert(config = config, expert_init_weights = expert_init_weights) for _ in range(num_experts)])
    ...
    def forward(
        ...
        dispatch_tensor = F.one_hot(input_routing, self.num_experts)
        expert_inputs = einsum('b n d, b n e -> b e n d', x, dispatch_tensor)
        ...
        output = einsum('b e n d, b n e -> b n d', expert_outputs, dispatch_tensor.float())
